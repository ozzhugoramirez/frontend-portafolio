...
export default function AdminProjectsPage() {
  ...
  if (loading && view === 'list') return <div className="p-6 md:p-12 font-mono text-xs text-gray-500 uppercase tracking-widest animate-pulse">Sincronizando The Forge...</div>;
      ...
      {view === 'list' && (
        <main className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
          <div className="border border-white/5 rounded-2xl overflow-x-auto bg-[#0d0d0f]/50 custom-scrollbar">
            <table className="w-full text-left text-[11px] font-mono min-w-[650px]">
              <thead className="bg-[#080809] text-gray-500 uppercase tracking-widest border-b border-white/5">
                <tr>
                  <th className="px-6 py-4 font-medium">Proyecto (Slug)</th>
              ...
              <tbody className="divide-y divide-white/5">
                {projects.map((p) => (
                  <tr key={p.slug} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-6 py-4"><div className="text-white font-medium text-sm md:text-[11px]">{p.title}</div><div className="text-gray-600 text-[9px] mt-1">/{p.slug}</div></td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleOpenForm(p)} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-all"><Edit3 size={16} /></button>
                        <button onClick={() => handleDeleteClick(p.slug)} className="p-2 hover:bg-red-500/10 rounded-lg text-gray-400 hover:text-red-500 transition-all"><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <form className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 custom-scrollbar pb-32">
            ...
            {activeTab === 'general' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 animate-in fade-in">
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 md:p-6 bg-[#0c0c0d] border border-red-900/30 rounded-2xl space-y-4">
                  ...
                  <div className="p-4 md:p-6 bg-[#0c0c0d] border border-white/5 rounded-2xl space-y-6">
                    <h3 className="text-xs font-mono text-emerald-500 flex items-center gap-2"><ImageIcon size={16}/> Media del Proyecto</h3>
                    
                    <div className="space-y-4">
                      <label className="text-[10px] font-mono text-gray-500 uppercase flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <span>Imagen Principal (Cover)</span>
                        {(imageFile || formData.image_main) && <span className="text-emerald-500/70">Preview Activa</span>}
                      </label>
                      
                      {(imageFile || formData.image_main) && (
                        <div className="w-full sm:max-w-[200px] aspect-video rounded-xl overflow-hidden border border-white/10 bg-black relative group">
                          <img 
                      ...
                      <input type="file" accept="image/*" onChange={(e) => e.target.files && setImageFile(e.target.files[0])} className="w-full text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-white/10 file:text-white hover:file:bg-white/20 transition-all cursor-pointer file:w-full sm:file:w-auto mt-2 sm:mt-0" />
                    </div>
                    ...
                    <div className="space-y-4 pt-6 border-t border-white/5">
                        ...
                        <div className="space-y-2 bg-black/20 p-3 rounded-xl border border-white/5">
                          ...
                          <div className="flex flex-wrap gap-3">
                            {formData.gallery.map((img: any, idx: number) => (
                              <div key={idx} className="w-14 h-14 md:w-16 md:h-16 rounded-lg overflow-hidden border border-white/10 bg-black relative group" title={img.caption || `Imagen ${idx+1}`}>
                                <img 
                        ...
                        <div className="space-y-2 bg-indigo-900/10 p-3 rounded-xl border border-indigo-500/20">
                          ...
                          <div className="flex flex-wrap gap-3">
                            {galleryFiles.map((file, idx) => (
                              <div key={idx} className="w-14 h-14 md:w-16 md:h-16 rounded-lg overflow-hidden border border-indigo-500/50 bg-black shadow-[0_0_10px_rgba(99,102,241,0.2)]">
                                <img 
                      ...
                      }} className="w-full text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-indigo-600/20 file:text-indigo-400 hover:file:bg-indigo-600/30 transition-all cursor-pointer file:w-full sm:file:w-auto mt-2 sm:mt-0" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'content' && (
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="analysis_text" value={formData.analysis_text} onChange={handleChange} rows={8} className="w-full bg-[#0c0c0d] border border-white/5 rounded-xl p-4 text-sm text-gray-300 outline-none focus:border-emerald-500/50 resize-y" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-mono text-indigo-500/50 uppercase px-2">Código Core</label>
                  <textarea name="core_code" value={formData.core_code} onChange={handleChange} rows={12} spellCheck="false" className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl p-4 text-[10px] md:text-[11px] font-mono text-gray-400 outline-none focus:border-indigo-500/50 resize-y" />
                </div>
              </div>
            )}
            ...
            {activeTab === 'data' && (
              <div className="space-y-8 md:space-y-12 animate-in fade-in">
                    ...
                    <BuilderListObject title="Métricas de Rendimiento" data={formData.metrics} onAdd={() => addObjectItem('metrics', {label: '', value: '', trend: ''})} onRemove={(idx: number) => removeObjectItem('metrics', idx)}
                      renderItem={(item: any, idx: number) => (
                        <div className="flex flex-col sm:grid sm:grid-cols-3 gap-2 w-full">
                          <input type="text" placeholder="Label" value={item.label} onChange={(e) => handleObjectArrayChange('metrics', idx, 'label', e.target.value)} className="bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white w-full" />
                          <input type="text" placeholder="Value" value={item.value} onChange={(e) => handleObjectArrayChange('metrics', idx, 'value', e.target.value)} className="bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white w-full" />
                          <input type="text" placeholder="Trend" value={item.trend} onChange={(e) => handleObjectArrayChange('metrics', idx, 'trend', e.target.value)} className="bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white w-full" />
                        </div>
                      )}
                    />
                    ...
                    <BuilderListObject title="Estructura de Carpetas" data={formData.structure} onAdd={() => addObjectItem('structure', {type: 'folder', name: '', desc: ''})} onRemove={(idx: number) => removeObjectItem('structure', idx)}
                      renderItem={(item: any, idx: number) => (
                        <div className="flex flex-col sm:grid sm:grid-cols-12 gap-2 w-full">
                          <select value={item.type} onChange={(e) => handleObjectArrayChange('structure', idx, 'type', e.target.value)} className="sm:col-span-4 bg-[#0c0c0d] text-xs p-3 sm:p-2 rounded outline-none border border-white/10 focus:border-indigo-500 text-gray-300 w-full">
                            <option value="folder">Folder</option>
                            <option value="file">File</option>
                          </select>
                          <input type="text" placeholder="/ruta" value={item.name} onChange={(e) => handleObjectArrayChange('structure', idx, 'name', e.target.value)} className="sm:col-span-8 bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white w-full" />
                          <input type="text" placeholder="Descripción corta" value={item.desc} onChange={(e) => handleObjectArrayChange('structure', idx, 'desc', e.target.value)} className="sm:col-span-12 bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white sm:mt-2 w-full" />
                        </div>
                      )}
                    />
                    ...
                    <BuilderListObject title="Pasos de Instalación" data={formData.install_steps} onAdd={() => addObjectItem('install_steps', {step: '', code: ''})} onRemove={(idx: number) => removeObjectItem('install_steps', idx)}
                      renderItem={(item: any, idx: number) => (
                        <div className="space-y-2 w-full">
                          <input type="text" placeholder="Paso (Ej: 1. Compilar)" value={item.step} onChange={(e) => handleObjectArrayChange('install_steps', idx, 'step', e.target.value)} className="w-full bg-white/5 text-xs p-3 sm:p-2 rounded outline-none border border-transparent focus:border-indigo-500 text-white" />
                          <input type="text" placeholder="Comando (Ej: make build)" value={item.code} onChange={(e) => handleObjectArrayChange('install_steps', idx, 'code', e.target.value)} className="w-full bg-[#0a0a0c] text-emerald-500 font-mono text-xs p-3 sm:p-2 rounded border border-gray-800 outline-none" />
                        </div>
                      )}
                    />
                  </div>
                ...
                <JSONArea label="Changelog (Historial - Formato JSON)" name="changelog" value={formData.changelog} onChange={handleChange} rows={8} />
              </div>
            )}
  ...
  return (
    <div className="bg-[#0c0c0d] border border-white/5 rounded-2xl p-4 md:p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button type="button" onClick={onAdd} className="bg-white/5 hover:bg-white/10 p-2 sm:p-1.5 rounded-md text-white transition-colors"><Plus size={14}/></button>
      </div>
      <div className="space-y-3">
        {safeData.map((item: string, idx: number) => (
          <div key={idx} className="flex gap-2 items-center">
            <span className="text-[10px] text-gray-600 font-mono hidden sm:inline">{idx + 1}.</span>
            <input type="text" value={item} onChange={(e) => onChange(idx, e.target.value)} placeholder={placeholder} className="flex-1 w-full bg-white/5 text-sm p-3 sm:p-2 rounded-lg outline-none border border-transparent focus:border-indigo-500/50 text-white" />
            <button type="button" onClick={() => onRemove(idx)} className="text-gray-600 hover:text-red-500 transition-colors p-3 sm:p-2 flex-shrink-0"><X size={16}/></button>
          </div>
        ))}
    ...
    <div className="bg-[#0c0c0d] border border-white/5 rounded-2xl p-4 md:p-6">
      ...
      <div className="space-y-4">
        {safeData.map((item: any, idx: number) => (
          <div key={idx} className="flex gap-3 items-start bg-black/20 p-4 sm:p-3 rounded-xl border border-white/5 w-full">
            <div className="flex-1 w-full">
              {renderItem(item, idx)} 
            </div>
            <button type="button" onClick={() => onRemove(idx)} className="text-gray-600 hover:text-red-500 transition-colors mt-1 p-1 flex-shrink-0"><X size={16}/></button>
          </div>
        ))}
        {safeData.length === 0 && <div className="text-[10px] text-gray-600 font-mono italic">No hay elementos cargados.</div>}
      </div>
